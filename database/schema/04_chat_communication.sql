-- =====================================================
-- AI CHAT & COMMUNICATION TABLES
-- =====================================================
-- This file contains all AI chat and communication related tables for SeraGPT
-- Tables: chat_sessions, chat_messages, ai_usage_logs, chat_templates, user_notifications
...
-- =====================================================
-- CHAT SESSIONS TABLE
-- Stores chat conversation sessions between users and AI
-- =====================================================
...
CREATE TABLE IF NOT EXISTS chat_sessions (
    ...
    -- Session context
    analysis_id UUID, -- Links to specific analysis if chat is about an analysis
    project_context JSONB DEFAULT '{}', -- Project-specific context data
    ...
    shared_token VARCHAR(100) UNIQUE, -- For public sharing
...
CREATE TABLE IF NOT EXISTS chat_messages (
    ...
    parent_message_id UUID REFERENCES chat_messages(id) ON DELETE SET NULL, -- For threaded conversations
    ...
    -- AI-specific fields
    ai_model VARCHAR(100), -- Model used for this specific message
    ...
    intent_classification VARCHAR(100), -- 'question', 'request', 'complaint', etc.
    ...
    references JSONB DEFAULT '[]', -- Referenced analyses, reports, etc.
    ...
    CONSTRAINT no_self_parent CONSTRAINT CHECK (id != parent_message_id),
...
-- =====================================================
-- AI USAGE LOGS TABLE
-- Detailed tracking of AI API usage and costs
-- =====================================================
...
CREATE TABLE IF NOT EXISTS ai_usage_logs (
    ...
    -- Cost tracking
    cost_per_token_input DECIMAL(10,8), -- Cost per input token in USD
    cost_per_token_output DECIMAL(10,8), -- Cost per output token in USD
    ...
    safety_scores JSONB DEFAULT '{}', -- Content safety scores
    ...
    feature_used VARCHAR(100), -- Which feature triggered this API call
    
    -- Token tracking for user balance
...
-- Pre-defined chat templates and prompts
-- =====================================================
...
CREATE TABLE IF NOT EXISTS chat_templates (
    ...
    tags TEXT[] DEFAULT '{}',
    language_code VARCHAR(10) DEFAULT 'tr-TR',
    industry_focus VARCHAR(100), -- 'greenhouse', 'agriculture', 'general'
...
CREATE TABLE IF NOT EXISTS user_notifications (
    ...
    -- Related entities
    related_entity_type VARCHAR(100), -- 'chat_session', 'payment', 'analysis', etc.
    ...
    -- Actions and buttons
    action_buttons JSONB DEFAULT '[]', -- Array of action button configs
    ...
    batch_id UUID, -- For batch sending
    
    -- Template information
    template_id VARCHAR(100), -- Email/push template used
    template_variables JSONB DEFAULT '{}',
    
    -- User preferences override
    respect_user_preferences BOOLEAN DEFAULT TRUE,
    force_send BOOLEAN DEFAULT FALSE, -- Override user notification settings
...
CREATE INDEX IF NOT EXISTS idx_chat_sessions_analysis_id ON chat_sessions(analysis_id) WHERE analysis_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_chat_sessions_shared ON chat_sessions(shared_token) WHERE is_shared = TRUE;
...
CREATE INDEX IF NOT EXISTS idx_ai_usage_logs_provider_model ON ai_usage_logs(provider, model_name);
CREATE INDEX IF NOT EXISTS idx_ai_usage_logs_created_at ON ai_usage_logs(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_ai_usage_logs_cost ON ai_usage_logs(total_cost_cents DESC);
CREATE INDEX IF NOT EXISTS idx_ai_usage_logs_tokens ON ai_usage_logs(total_tokens DESC);
...
CREATE INDEX IF NOT EXISTS idx_chat_templates_active ON chat_templates(is_active) WHERE is_active = TRUE;
CREATE INDEX IF NOT EXISTS idx_chat_templates_public ON chat_templates(is_public) WHERE is_public = TRUE;
CREATE INDEX IF NOT EXISTS idx_chat_templates_created_by ON chat_templates(created_by);
CREATE INDEX IF NOT EXISTS idx_chat_templates_usage_count ON chat_templates(usage_count DESC);
...
CREATE INDEX IF NOT EXISTS idx_user_notifications_group ON user_notifications(group_key) WHERE group_key IS NOT NULL;
...
-- Insert sample chat templates (commented out for production)
/*
INSERT INTO chat_templates (
    name, description, category, system_prompt, user_prompt_template,
    variables, required_variables, is_public, is_active
) VALUES 
(
    'ROI Analysis Template',
    'Template for ROI analysis conversations',
    'roi_analysis',
    'You are an expert agricultural investment advisor specializing in greenhouse projects. Provide detailed ROI analysis based on user inputs.',
    'I want to analyze the ROI for a greenhouse project with the following details: Location: {{location}}, Crop: {{crop}}, Size: {{size}} m², Investment Budget: {{budget}} TL. Please provide a comprehensive analysis.',
    '[
        {"name": "location", "type": "string", "description": "Project location"},
        {"name": "crop", "type": "string", "description": "Crop type to be grown"},
        {"name": "size", "type": "number", "description": "Greenhouse size in square meters"},
        {"name": "budget", "type": "number", "description": "Investment budget in Turkish Lira"}
    ]'::jsonb,
    ARRAY['location', 'crop', 'size', 'budget'],
    TRUE,
    TRUE
);
*/
...
COMMENT ON TABLE chat_sessions IS 'Stores chat conversation sessions between users and AI with context and configuration';
